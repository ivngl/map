...
function Map3() {
  ...
  const fetchVacancies = async (regionName: string): Promise<any | null> => {
    try {
      ...
      return data.found//Promise.resolve(500)
      //data.total
    ...
    return (
      <svg ref={mapRef} width="1920" height="1080">
        {MOCK_REGIONS.map((region, idx) => {
          ...
          const text = String(region.vacancies)//vacancies?.map(item => `${item.name}: ${item.count}`).join('\n') ?? '';
